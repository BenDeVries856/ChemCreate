...
# TODO - make it so you cant erase the center
...
class Atom:
  ...
  def draw(self, draw_circle):
      if self.highlighted:
        ...
        pygame.draw.circle(screen, COLOR_BLUE, (self.x, self.y), self.osize)
        text_surface = font_molecule.render('1', True, COLOR_WHITE)
        screen.blit(text_surface, dest=(self.x-self.size+3, self.y-self.size))
      elif self.element == "n":
        pygame.draw.circle(screen, COLOR_GREEN, (self.x, self.y), self.osize)
        text_surface = font_molecule.render('2', True, COLOR_WHITE)
        screen.blit(text_surface, dest=(self.x-self.size+3, self.y-self.size))
      elif self.element == "o":
        pygame.draw.circle(screen, COLOR_RED, (self.x, self.y), self.osize)
        text_surface = font_molecule.render('3', True, COLOR_WHITE)
        screen.blit(text_surface, dest=(self.x-self.size+3, self.y-self.size))
      elif self.element == "h":
        pygame.draw.circle(screen, COLOR_BLACK, (self.x, self.y), self.osize)
        text_surface = font_molecule.render('4', True, COLOR_WHITE)
        screen.blit(text_surface, dest=(self.x-self.size+3, self.y-self.size))
      elif draw_circle:
        pygame.draw.circle(screen, COLOR_BLACK, (self.x, self.y), self.size)
...
class Menu:
  def __init__(self):
    ...
    self.buttons.append(MenuButton("single", True, self.x + (BUTTON_SIZE*10), self.y + (self.height/2)))
    self.buttons.append(MenuButton("wedge", False, self.x + (BUTTON_SIZE*12), self.y + (self.height/2)))
    self.buttons.append(MenuButton("dash", False, self.x + (BUTTON_SIZE*14), self.y + (self.height/2)))
...
class MenuButton:
  ...
  def draw(self):
    ...
    if self.function == "carbon":
      pygame.draw.circle(screen, COLOR_BLUE, (self.x+(BUTTON_SIZE/2), self.y+(BUTTON_SIZE/2)-1), (BUTTON_SIZE/2))
      text_surface = font_molecule.render('1', True, COLOR_WHITE)
      screen.blit(text_surface, dest=(self.x+(BUTTON_SIZE/4), self.y))
    elif self.function == "nitrogen":
      pygame.draw.circle(screen, COLOR_GREEN, (self.x+(BUTTON_SIZE/2), self.y+(BUTTON_SIZE/2)-1), (BUTTON_SIZE/2))
      text_surface = font_molecule.render('2', True, COLOR_WHITE)
      screen.blit(text_surface, dest=(self.x+(BUTTON_SIZE/4), self.y))
    elif self.function == "oxygen":
      pygame.draw.circle(screen, COLOR_RED, (self.x+(BUTTON_SIZE/2), self.y+(BUTTON_SIZE/2)-1), (BUTTON_SIZE/2))
      text_surface = font_molecule.render('3', True, COLOR_WHITE)
      screen.blit(text_surface, dest=(self.x+(BUTTON_SIZE/4), self.y))
    elif self.function == "hydrogen":
      pygame.draw.circle(screen, COLOR_BLACK, (self.x+(BUTTON_SIZE/2), self.y+(BUTTON_SIZE/2)-1), (BUTTON_SIZE/2))
      text_surface = font_molecule.render('4', True, COLOR_WHITE)
      screen.blit(text_surface, dest=(self.x+(BUTTON_SIZE/4), self.y))
    # draw icon
